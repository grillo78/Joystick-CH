...
public class RenderUtil {
    ...
    public static void transformLevel(PoseStack pPoseStack, float partialTicks, ShipRotations shipRotations) {
        if (!Minecraft.getInstance().gameRenderer.getMainCamera().isDetached()) {
            float viewOffset = (float) (Minecraft.getInstance().player.getEyeHeight() - Minecraft.getInstance().player.getVehicle().getPassengersRidingOffset() * 2.18);
            pPoseStack.mulPose(new Quaternionf().rotationX((float) Math.toRadians(Mth.lerp(Minecraft.getInstance().getFrameTime(), Minecraft.getInstance().player.xRotO, Minecraft.getInstance().player.getXRot()))));
            pPoseStack.mulPose(new Quaternionf().rotationY((float) Math.toRadians(Mth.lerp(Minecraft.getInstance().getFrameTime(), Minecraft.getInstance().player.yHeadRotO, Minecraft.getInstance().player.yHeadRot))));
            ...
            pPoseStack.mulPose(shipRotations.getRotations().conjugate(new Quaternionf()).nlerp(shipRotations.getRotations().conjugate(new Quaternionf()), partialTicks));
            pPoseStack.mulPose(new Quaternionf().rotationY((float) Math.toRadians(180)));
        }
    }
}
